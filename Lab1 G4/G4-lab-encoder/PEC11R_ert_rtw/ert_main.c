...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) PEC11R_M->Timing.taskTime0;
  PEC11R_step();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(PEC11R_M->extModeInfo, &rtmGetTFinal(PEC11R_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(PEC11R_M, true);
    }
  }
