...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) AMT103_M->Timing.taskTime0;
  AMT103_step();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(AMT103_M->extModeInfo, &rtmGetTFinal(AMT103_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(AMT103_M, true);
    }
  }
